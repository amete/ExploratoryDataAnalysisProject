...
result <- summarise(group_by(NEI_Baltimore,year),sum(Emissions,na.rm = TRUE))
result <- as.data.frame(result)

# Plot the data
png("plot2.png", width=640, height=480, units = "px", type="quartz")
plot(result, xaxt="n", main = expression(paste("Total ","PM"[2.5]," Emission in Baltimore (1999-2008)")), 
     xlab = "Year", ylab = expression(paste("Total ","PM"[2.5]," Emission (tons)")),
     pch = 19, cex = 1.2, col = "blue", ylim=c(1.e+03,4.e+03))
